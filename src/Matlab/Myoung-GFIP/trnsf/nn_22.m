...
% Use this for even n:
% 11,12,13,14,15,18,19,23,29,31,103,104,201,202,203
[n2,tem]=size(x);
n=n2/2;
m=n/2;
l=n/4;
...
%
if (gp==13) % sdp ([-1 0;0 -1] x [1 1;0 -1]) on characters
   d1=2;
   c1(1,1)=1;
   c1(2,1)=1;
   c1(1,2)=1+m;
...
%
if (gp==15) % sdp ([0 -1;-1 0] x [0 m+1;m+1 0])
   d1=2;
   c1(1,1)=1;
   c1(2,1)=1;
   c1(1,2)=m+1;
...
%
if (gp==18) % sdp ([m+1 0;0 m+1] x [-1 1;0 1])
   d1=m;
   for k=0:l-1
      c1(1,k+1)=2*k+1;
      c1(2,k+1)=4*k+1;
...
%
for k=1:d1
   t1(1,1,k)=bf(c1(1,k),c1(2,k));
   t1(2,1,k)=bf(c1(1,k)+n,c1(2,k));
   t1(1,2,k)=bf(c1(1,k),c1(2,k)+n);
   t1(2,2,k)=bf(c1(1,k)+n,c1(2,k)+n);
end      
t1=2*ifft2(t1);
for k=1:d1
   bf(c1(1,k),c1(2,k))=t1(1,1,k);
   bf(c1(1,k),c1(2,k)+n)=t1(1,2,k);
   bf(c1(1,k)+n,c1(2,k))=t1(2,1,k);
   bf(c1(1,k)+n,c1(2,k)+n)=t1(2,2,k);
   y1(c1(1,k),c1(2,k))=t1(1,1,k);
   y1(c1(1,k),c1(2,k)+n)=t1(1,2,k);
   y1(c1(1,k)+n,c1(2,k))=t1(2,1,k);
   y1(c1(1,k)+n,c1(2,k)+n)=t1(2,2,k);
end
for k=1:d21
