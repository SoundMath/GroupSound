...
if (gp==101)                    %[0 m-1;-1 m]
   ...
   for k1=0:m-1
      for k2=0:m-1
         c2(1,1+k1+m*k2)=2*k1+1;
         c2(2,1+k1+m*k2)=2*k2+1;
         c2(1,1+k1+m*k2+m*m)=2*k1+2;
         c2(2,1+k1+m*k2+m*m)=2*k2+2;
      end
   end
   toc
   %
   ...
   cc2(1,1:m*m)=repmat(ev,1,m);
   cc2(2,1:m*m)=kron(ev,ones(1,m));
   cc2(1,1+m*m:n*m)=repmat(od,1,m);
   cc2(2,1+m*m:n*m)=kron(od,ones(1,m));
   toc
end   
%
%
...
for k=1:d2
   ...
   s2(2,k)=bf(c2(1,k)+n,c2(2,k)+n);
                     ...
                     % Note: a file called tem.m 
                     % contained the nn_4.m function
                     % but with these 2 lines changed:
t2=sqrt(2)*ifft(t2); % --> t2=f2(t2,d2);
s2=sqrt(2)*ifft(s2); % --> s2=f2(s2,d2);
                     % But tem.m didn't handle gp=101
                     % case (above)
for k=1:d2
   k1=c2(1,k);
   k2=c2(2,k);
