function y=inn_22(x,gp)
% Use this for even n:
% NB file/fctn name mismatch: fctn name was inn2_22(x,gp)
[n2,tem]=size(x);
n=n2/2;
m=n/2;
l=n/4;
...
%
for k=1:d1
   t1(1,1,k)=x(c1(1,k),c1(2,k));
   t1(2,1,k)=x(c1(1,k)+n,c1(2,k));
   t1(1,2,k)=x(c1(1,k),c1(2,k)+n);
   t1(2,2,k)=x(c1(1,k)+n,c1(2,k)+n);
end      
t1=.5*fft2(t1);
for k=1:d1
   x(c1(1,k),c1(2,k))=t1(1,1,k);
   x(c1(1,k)+n,c1(2,k))=t1(2,1,k);
   x(c1(1,k),c1(2,k)+n)=t1(1,2,k);
   x(c1(1,k)+n,c1(2,k)+n)=t1(2,2,k);
end
for k=1:d21
